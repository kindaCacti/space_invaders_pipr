...
def shoot_entities(entities: list, bullets):
    if len(entities) == 0:
        return
...
def show_text(screen, content, position, size = 24, color = "white"):
...
def show_best_score(screen, best_score):
    show_text(screen, f"highscore: {max(best_score, score)}", (20, 40))

def show_middle_text(screen, content, position, size = 24, color = "white"):
    font = pygame.font.Font(None, size)
    image = font.render(content, True, color)
...
def show_button(screen, position, content, size):
    pygame.draw.rect(screen, "white", [position[0], position[1], size[0], size[1]])
    show_middle_text(screen, content, (position[0]+size[0]/2, position[1]+size[1]/2), color = "black")
    return [position[0], position[1], position[0]+size[0], position[1]+size[1]]

def is_within(point, end_points):
...
def check_end_condition(players, enemies):
...
def show_lose_screen(score):
    ...
    while running:
        ...
        show_middle_text(screen, "YOU LOOSE", (300, 100), size = 50)
...
def show_game_window():
    ...
    while running:
        ...
        if keys[pygame.K_w] and not player_shot:
            shoot_entities(players, bullets)
            player_shot = True
